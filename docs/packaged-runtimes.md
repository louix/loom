...
additional provider runtimes remain follow-up work. Networked command MCPs are
deferred until a concrete stdio-only server needs them; current networked MCPs
use the HTTP worker. Claude retains its provider-specific egress proxy.
