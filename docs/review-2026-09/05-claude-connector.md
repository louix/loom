...
- **Issue:** `start()` records `this.#startOpts = opts` once, from the original
  ...
  call and then does an undo, the resumed session silently runs the *original*
...
- **Issue:** The mapper is deliberately kept across a rewind and
  ...
  source here) is *cumulative across turns of one `query()` call* and
  ...
  first post-rewind turn, and then it *overwrites* `state.usage` /
...
- **Fix:** Serialize all `forkSession` calls process-wide behind a single mutex,
  ...
  (`SessionMutationOptions.dir` is the *project* dir, not the config dir, so it
...
- **Issue:** `rewind()` sets `#rewindInFlight = true` but only sets
  `#rewinding = true` *after* `#forkTruncated` resolves. During the fork
...
- **Issue:** `interrupt()` sets `#interrupted`, drains `#inbox`, and calls
  ...
  noted). But:
  (a) `#interrupted` only suppresses *mapper* events in `#drain`;
...
- **`modelUsage` differencing in steady state** (map.ts:375–391): correct —
  ...
  fallback to per-turn `m.usage` is the right shape *within a single query*.
