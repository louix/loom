...
- **Severity:** medium
- **Issue:** `#bringUp()` acquires the pidfile first (line 359) but installs `SIGINT`/`SIGTERM` handlers only near the end (line 383), *after* `#resolveAutoModels()` and `#resolveClaudeModels()`, which each do bounded-but-real network I/O (up to ~10s for the Claude probe). Two problems in that window:
...
- **Severity:** low-medium
- **Issue:** For a stale-epoch child it sends `SIGTERM`, schedules an `unref`'d `setTimeout(2000)` for `SIGKILL`, then immediately `children.forget(row.pid)` and counts it reaped. If the child ignores `SIGTERM` and the new daemon exits (or is itself killed) within ~2s, the `unref`'d timer never fires, the child survives, and its ChildStore row is already gone — so the *next* restart's `reapChildren` has nothing to act on and the process is orphaned permanently. The "reaped" count in the hygiene report also overstates what actually terminated.
