...
| Bash (run_in_background) / BashOutput / KillShell | **shipped** — `tools/background.ts` `BackgroundTasks`: detached `bash -c` tasks with their own process group, live-clamped per-task unread buffer, read cursor + regex filter + wait, default 10 min timeout (`timeout_ms: 0` = none) |
...
| git commit / ask_user / status                    | the existing in-process `loom` MCP server (commit + status from `@loom/core`), exposed to the SDK as tools via `mcp.ts`; aisdk sessions get the same via `loom-tools.ts`                                                              |
