...
export const decodeManifest = (value: unknown): RuntimeManifest => {
  ...
  if (
    Object.keys(v).some((k) => !["version", "system", "backend", "entrypoint", "args"].includes(k))
  )
...
export const resolveRuntime = async (source: string, home = runtimeHome()) => {
  ...
  try {
    ...
    if (!(await Deno.stat(lock.smolvm)).isFile) throw new Error("Prepared smolvm is missing");
