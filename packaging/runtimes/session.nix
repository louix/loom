# Common session tools; credentials and writable state are injected per launch.
{ pkgs, loom }:
let
  providerPath = pkgs.lib.makeBinPath [ pkgs.claude-code pkgs.codex ];
  package = pkgs.writeShellScriptBin "loom-session" ''
    ...
    max-jobs = 1
    cores = 1'
    export LOOM_GUEST_PROVIDER_PATH=${providerPath}
