...
test(
  "legacy host tilth definitions still speak MCP on stdio",
  { skip: onPath("tilth") ? false : "tilth is not installed" },
  async () => {
    const mount = normalizeConfig({
      local_tools: { tilth: { command: "tilth", args: ["--mcp", "--edit"] } },
      session: { local_tools: ["tilth"] },
    }).mcp[0]!;
